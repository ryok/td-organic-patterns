...
# 細部の濃さ = 0.35 + 高域*12（0〜1 にクランプ）。aud_out が無ければ高域項は 0。
# 高域は修正後の帯域で p95 が約 0.018 と小さいため、12 倍で p95 時に +0.22（0.35→0.57）
# 動く。音楽反応側の高域ゲインと同じ比（×4.82）で較正した（2026-09-27、実曲で計測）。
DETAIL_TERM = f"{pbus.ch('aud_out', 'high')}*12"
